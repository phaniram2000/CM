namespace Dreamteck.Editor
{
    ...
    public class ModuleInstaller
    {
        ...
        public void AddAssemblyLink(string dreamteckAssemblyDirectory, string dreamteckAssemblyName,
            string addedAssemblyName)
        {
            string localFilePath = Path.Combine(DREAMTECK_FOLDER_NAME, dreamteckAssemblyDirectory,
        ...
        {
            string globalPath =
                ResourceUtility.FindFolder(Application.dataPath, DREAMTECK_FOLDER_NAME + "/" + _packageDirectory);
            if (!Directory.Exists(globalPath))
            {
                EditorUtility.DisplayDialog("Missing Package", "Package directory not found: " + _packageDirectory,
                    "OK");
                return;
            }
            ...
            {
                ScriptingDefineUtility.Add(_scriptingDefines[i], EditorUserBuildSettings.selectedBuildTargetGroup,
                    true);
            }
        }
        ...
        public void Uninstall()
        {
            ...
            for (int i = 0; i < _uninstallDirectories.Count; i++)
            {
                string globalPath = ResourceUtility.FindFolder(Application.dataPath,
                    DREAMTECK_FOLDER_NAME + "/" + _uninstallDirectories[i]);
            ...
            for (int i = 0; i < _scriptingDefines.Count; i++)
            {
                ScriptingDefineUtility.Remove(_scriptingDefines[i], EditorUserBuildSettings.selectedBuildTargetGroup,
                    true);
            }
        }
