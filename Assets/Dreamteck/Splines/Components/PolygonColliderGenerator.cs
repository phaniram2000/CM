...
namespace Dreamteck.Splines
{
    ...
    public class PolygonColliderGenerator : SplineUser
    {
        ...
        [SerializeField] [HideInInspector] private Type _type = Type.Path;
        [SerializeField] [HideInInspector] private float _size = 1f;
        [SerializeField] [HideInInspector] private float _offset = 0f;
        [SerializeField] [HideInInspector] protected PolygonCollider2D polygonCollider;

        [SerializeField] [HideInInspector] protected Vector2[] vertices = new Vector2[0];
        ...
        private void GenerateShape()
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                if (offset != 0f)
                {
                    Vector2 right = new Vector2(-evalResult.forward.y, evalResult.forward.x).normalized *
                                    evalResult.size;
