...
namespace Dreamteck.Splines
{
    ...
    public class SurfaceGenerator : MeshGenerator
    {
        ...
        [SerializeField] [HideInInspector] private float _expand = 0f;
        [SerializeField] [HideInInspector] private float _extrude = 0f;
        [SerializeField] [HideInInspector] private Vector2 _sideUvScale = Vector2.one;
        [SerializeField] [HideInInspector] private Vector2 _sideUvOffset = Vector2.zero;
        [SerializeField] [HideInInspector] private float _sideUvRotation = 0f;
        [SerializeField] [HideInInspector] private SplineComputer _extrudeSpline;
        [SerializeField] [HideInInspector] private Vector3 _extrudeOffset = Vector3.zero;
        [SerializeField] [HideInInspector] private SplineSample[] extrudeResults = new SplineSample[0];
        [SerializeField] [HideInInspector] private Vector3[] identityVertices = new Vector3[0];
        [SerializeField] [HideInInspector] private Vector3[] identityNormals = new Vector3[0];
        [SerializeField] [HideInInspector] private Vector2[] projectedVerts = new Vector2[0];
        [SerializeField] [HideInInspector] private int[] surfaceTris = new int[0];
        [SerializeField] [HideInInspector] private int[] wallTris = new int[0];

        [SerializeField] [HideInInspector] private double _extrudeFrom = 0.0;
        [SerializeField] [HideInInspector] private double _extrudeTo = 1.0;
        [SerializeField] [HideInInspector] private bool _uniformUvs = false;
        ...
        public void Generate()
        {
            ...
            {
                _tsMesh.uv[i].x = Mathf.InverseLerp(max.x, min.x, projectedVerts[i].x) * uvScale.x - uvScale.x * 0.5f +
                                  uvOffset.x + 0.5f;
                _tsMesh.uv[i].y = Mathf.InverseLerp(min.y, max.y, projectedVerts[i].y) * uvScale.y - uvScale.y * 0.5f +
                                  uvOffset.y + 0.5f;
            ...
            if (pathExtrude)
            {
                ...
                for (int i = 0; i < surfaceVertexCount; i++)
                {
                    ...
                    _tsMesh.vertices[i + surfaceVertexCount] = extrudeResults[0].position +
                                                               (extrudeResults[0].rotation * identityVertices[i] +
                                                                off) + vertexOffset;
                    ...
                    _tsMesh.vertices[i] = extrudeResults[extrudeResults.Length - 1].position +
                                          (extrudeResults[extrudeResults.Length - 1].rotation * identityVertices[i] +
                                           off) + vertexOffset;
                ...
                for (int i = 0; i < extrudeResults.Length; i++)
                {
                    ...
                    for (int n = 0; n < identityVertices.Length; n++)
                    {
                        ...
                        _tsMesh.vertices[startIndex + n] = extrudeResults[i].position +
                                                           (extrudeResults[i].rotation * identityVertices[n] + off) +
                                                           vertexOffset;
                        _tsMesh.normals[startIndex + n] = extrudeResults[i].rotation * identityNormals[n];
                        if (_uniformUvs)
                        {
                            _tsMesh.uv[startIndex + n] =
                                new Vector2((float)n / (identityVertices.Length - 1) * _sideUvScale.x + _sideUvOffset.x,
                                    totalLength * _sideUvScale.y + _sideUvOffset.y);
                        }
                        else
                        {
                            _tsMesh.uv[startIndex + n] =
                                new Vector2((float)n / (identityVertices.Length - 1) * _sideUvScale.x + _sideUvOffset.x,
                                    (float)i / (extrudeResults.Length - 1) * _sideUvScale.y + _sideUvOffset.y);
                        }

                        if (_sideUvRotation != 0f)
                        {
                            _tsMesh.uv[startIndex + n] = Quaternion.AngleAxis(_sideUvRotation, Vector3.forward) *
                                                         _tsMesh.uv[startIndex + n];
                        }
                ...
                MeshUtility.GeneratePlaneTriangles(ref wallTris, sampleCount - 1, extrudeResults.Length, flipSide, 0, 0,
                    true);
            ...
            else if (simpleExtrude)
            {
                ...
                for (int i = 0; i < surfaceVertexCount + 1; i++)
                {
                    ...
                    _tsMesh.uv[i + surfaceVertexCount * 2] = new Vector2(
                        (float)i / (surfaceVertexCount - 1) * _sideUvScale.x + _sideUvOffset.x, 0f + _sideUvOffset.y);
                    if (clockwise)
                    {
                    ...
                    if (_uniformUvs)
                    {
                        _tsMesh.uv[offsetIndex] =
                            new Vector2((float)i / surfaceVertexCount * _sideUvScale.x + _sideUvOffset.x,
                                _extrude * _sideUvScale.y + _sideUvOffset.y);
                    }
                    else
                    {
                        _tsMesh.uv[offsetIndex] =
                            new Vector2((float)i / surfaceVertexCount * _sideUvScale.x + _sideUvOffset.x,
                                1f * _sideUvScale.y + _sideUvOffset.y);
                    }

                    if (_sideUvRotation != 0f)
                    {
                        _tsMesh.uv[offsetIndex] = Quaternion.AngleAxis(_sideUvRotation, Vector3.forward) *
                                                  _tsMesh.uv[offsetIndex];
                    }
        ...
        bool IsClockwise(Vector2[] points2D)
        {
            ...
            sum += (points2D[0].x - points2D[points2D.Length - 1].x) *
                   (points2D[0].y + points2D[points2D.Length - 1].y);
            return sum <= 0f;
        }
        ...
        void GetIdentityVerts(Vector3 center, Vector3 normal, bool clockwise)
        {
            ...
            for (int i = 0; i < sampleCount; i++)
            {
                ...
                identityVertices[i] =
                    vertsRotation * (evalResult.position - center + (clockwise ? -right : right) * _expand);
