...
namespace Dreamteck.Splines
{
    ...
    public class PathGenerator : MeshGenerator
    {
        ...
        [SerializeField] [HideInInspector] private bool _useShapeCurve = false;
                    ...
                    if (_slices > 1)
                    {
                        if (n < _slices)
                        {
                            ...
                            Vector3 cross1 = -Vector3
                                .Cross(evalResult.forward, nextVertPos - _tsMesh.vertices[vertexIndex]).normalized;

                            if (n > 0)
                            {
                                Vector3 cross2 = -Vector3.Cross(evalResult.forward,
                                    _tsMesh.vertices[vertexIndex] - lastVertPos).normalized;
