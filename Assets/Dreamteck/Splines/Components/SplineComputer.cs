namespace Dreamteck.Splines
{
    ...
    public partial class SplineComputer : MonoBehaviour
    {
        ...
        [HideInInspector] public bool editorDrawPivot = true;
        [HideInInspector] public Color editorPathColor = Color.white;
        [HideInInspector] public bool editorAlwaysDraw = false;
        [HideInInspector] public bool editorDrawThickness = false;
        [HideInInspector] public bool editorBillboardThickness = true;
        private bool _editorIsPlaying = false;
        [HideInInspector] public bool isNewlyCreated = true;
        [HideInInspector] public EditorUpdateMode editorUpdateMode = EditorUpdateMode.Default;
#endif
        public enum Space
        {
            World,
        ...
        [HideInInspector] public bool multithreaded = false;
        [HideInInspector] public UpdateMode updateMode = UpdateMode.Update;
        [HideInInspector] public TriggerGroup[] triggerGroups = new TriggerGroup[0];
        ...
        [HideInInspector] [SerializeField] [UnityEngine.Serialization.FormerlySerializedAs("spline")]
        private Spline _spline = new Spline(Spline.Type.CatmullRom);

        [HideInInspector] private SampleCollection _sampleCollection = new SampleCollection();

        [HideInInspector] [SerializeField] [UnityEngine.Serialization.FormerlySerializedAs("originalSamplePercents")]
        private double[] _originalSamplePercents = new double[0];

        [HideInInspector] [SerializeField] private bool _is2D = false;
        [HideInInspector] [SerializeField] private bool hasSamples = false;

        [HideInInspector] [SerializeField] [Range(0.001f, 45f)]
        private float _optimizeAngleThreshold = 0.5f;

        [HideInInspector] [SerializeField] private Space _space = Space.Local;
        [HideInInspector] [SerializeField] private SampleMode _sampleMode = SampleMode.Default;
        [HideInInspector] [SerializeField] private SplineUser[] _subscribers = new SplineUser[0];

        [HideInInspector] [SerializeField] private SplineSample[] _rawSamples = new SplineSample[0];
        ...
        [HideInInspector] [SerializeField] [UnityEngine.Serialization.FormerlySerializedAs("nodes")]
        ...
        public double GetPointPercent(int pointIndex)
        {
            ...
            for (int i = _originalSamplePercents.Length - 2; i >= 0; i--)
            {
                ...
                {
                    double inverseLerp = DMath.InverseLerp(_originalSamplePercents[i], _originalSamplePercents[i + 1],
                        percent);
        ...
        private void CalculateSamples(bool transformSamples = true)
        {
            ...
            if (_sampleMode == SampleMode.Optimized)
            {
                ...
                if (_rawSamples.Length > 2)
                {
                    Vector3 lastDirection = _rawSamples[0].forward;
                    List<SplineSample> optimized = new List<SplineSample>();
                    for (int i = 0; i < _rawSamples.Length; i++)
                    {
                        Vector3 direction = _rawSamples[i].forward;
                        if (i < _rawSamples.Length - 1)
                        {
                            direction = _rawSamples[i + 1].position - _rawSamples[i].position;
                        }
                    ...
                    _rawSamples = optimized.ToArray();
                }
                else
                {
                    for (int i = 0; i < _sampleCollection.optimizedIndices.Length; i++)
                    {
                        _sampleCollection.optimizedIndices[i] = i;
                    }
                }
            }
            else
        ...
        {
            [SerializeField] internal Node node = null;
            [SerializeField] internal int pointIndex = 0;
