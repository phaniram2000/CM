...
namespace Dreamteck.Splines
{
    ...
    {
        //Mesh data
        [SerializeField] [HideInInspector, UnityEngine.Serialization.FormerlySerializedAs("channels")]
                ...
                switch (_channels[i].type)
                {
                    ...
                    case Channel.Type.Place:
                        for (int j = 0; j < _channels[i].count; j++)
                        {
                            ...
                            Place(_channels[i], _combineMeshes[combineMeshIndex],
                                DMath.Lerp(_channels[i].clipFrom, _channels[i].clipTo,
                                    (double)j / Mathf.Max(_channels[i].count - 1, 1)));
                            combineMeshIndex++;
                        }
        ...
        private void Place(Channel channel, TS_Mesh target, double percent)
        {
            ...
            _vertexMatrix.SetTRS(
                evalResult.position + originalRight * (finalOffset.x * resultSize) +
        ...
        private void Extrude(Channel channel, TS_Mesh target, double from, double to)
        {
            ...
            for (int i = 0; i < definition.vertexGroups.Count; i++)
            {
                ...
                _vertexMatrix.SetTRS(
                    _modifiedResult.position + originalRight * (finalOffset.x * resultSize) +
                    originalNormal * (finalOffset.y * resultSize) + originalDirection * offset.z, //Position
                    ...
                    if (target.uv.Length > index)
                    {
                        ...
                        switch (channel.overrideUVs)
                        {
                            case Channel.UVOverride.ClampU:
                                uv.x = (float)_modifiedResult.percent;
                                break;
                            case Channel.UVOverride.ClampV:
                                uv.y = (float)_modifiedResult.percent;
                                break;
                            case Channel.UVOverride.UniformU:
                                uv.x = CalculateLength(0.0, ClipPercent(_modifiedResult.percent));
                                break;
                            case Channel.UVOverride.UniformV:
                                uv.y = CalculateLength(0.0, ClipPercent(_modifiedResult.percent));
                                break;
                        }
