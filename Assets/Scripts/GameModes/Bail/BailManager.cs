...
public class BailManager : MonoBehaviour
{
    ...
    public void OnMoveSeq()
    {
        ...
        seq.AppendCallback(() =>
        {
            Virtualcam.GetComponent<Animator>().SetTrigger(("Chair"));
        });
        seq.AppendInterval(0.1f);
        seq.AppendCallback(() =>
        {
            Playeranim.SetTrigger("Typing");
            
        }); 
        seq.AppendInterval(1f);
        seq.AppendCallback(() =>
        {
            Virtualcam.GetComponent<Animator>().SetTrigger(("Laptop"));
        });
    ...
    public void Prisionerseq()
    {
        ...
        seq.AppendCallback(() =>
        {
            Virtualcam.GetComponent<Animator>().SetTrigger("Prisioner");
        ...
        seq.Append(Prisioner.transform.DOMove(PrisionerMovePosition.transform.position, 3f).SetEase(Ease.Linear).OnComplete(
            () =>
            {
                if (AudioManager.instance)
                {
    ...
    public void Bailbutton()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            if (Physics.Raycast(ray, out hitinfo) && hitinfo.transform.name=="Bail Button")
            {
                ...
                BailButton.transform.DOMoveY(BailButton.transform.position.y-0.05f, 0.1f).SetEase(Ease.Linear).OnComplete(() =>
                {
                    BailButton.transform.DOMoveY(BailButton.transform.position.y+0.05f, 0.1f).SetEase(Ease.Linear).OnComplete(
                        () =>
                        {
