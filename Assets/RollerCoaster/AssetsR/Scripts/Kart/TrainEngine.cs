...
namespace Kart
{
	public class TrainEngine : MonoBehaviour
	{
		...
		{
			var node = passed[0].node; //Get the node of the junction
			var junctionSwitch = node.GetComponent<JunctionSwitch>(); //Look for a JunctionSwitch component
			if (!junctionSwitch) return; //No JunctionSwitch - ignore it - this isn't a real junction
		...
		private void SwitchSpline(Node.Connection from, Node.Connection to)
		{
			...
			var excessDistance = from.spline.CalculateLength(from.spline.GetPointPercent(from.pointIndex),
			...
			var startpercent = _tracer.ClipPercent(to.spline.GetPointPercent(to.pointIndex));
			if (Vector3.Dot(from.spline.Evaluate(from.pointIndex).forward, to.spline.Evaluate(to.pointIndex).forward) <
				0f)
			{
